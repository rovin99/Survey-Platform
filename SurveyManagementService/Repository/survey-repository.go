...
func (r *surveyRepository) GetByID(ctx context.Context, id uint) (*models.Survey, error) {
	...
	err := r.db.WithContext(ctx).Preload("Questions.Options").Preload("Questions").First(&survey, id).Error
	return &survey, err
}
...
func (r *surveyRepository) GetByIDWithTx(ctx context.Context, tx *gorm.DB, id uint) (*models.Survey, error) {
	var survey models.Survey
	err := tx.WithContext(ctx).Preload("Questions.Options").Preload("Questions").First(&survey, id).Error
	return &survey, err
}
