...
func NewAuthService(baseURL string) *AuthService {
    if _, err := url.ParseRequestURI(baseURL); err != nil {
        panic(fmt.Sprintf("Invalid base URL for AuthService: %s", baseURL))
    }
...
func (s *AuthService) AssignConductorRole(userID uint) error {
    ...
    if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
        ...
        if err := json.NewDecoder(resp.Body).Decode(&errorResponse); err != nil {
            return fmt.Errorf("failed to assign conductor role: status %d, unable to decode error response", resp.StatusCode)
        }
        return fmt.Errorf("failed to assign conductor role: status %d, error: %v", resp.StatusCode, errorResponse)
    }
