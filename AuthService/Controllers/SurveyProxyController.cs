...
namespace AuthService.Controllers
{
    ...
    public class SurveyProxyController : ControllerBase
    {
        ...
        private async Task<string> GenerateBearerToken()
        {
            ...
            // Look up conductor ID if user has Conducting role
            string conductorIdStr = null;
            if (roles.Contains("Conducting") && int.TryParse(userId, out int userIdInt))
            {
                var conductor = await _dbContext.Conductors
                    .FirstOrDefaultAsync(c => c.UserId == userIdInt);
                if (conductor != null)
                {
                    conductorIdStr = conductor.ConductorId.ToString();
                    _logger.LogInformation("Found conductorId {ConductorId} for userId {UserId}",
                        conductorIdStr, userId);
                }
            }
            ...
            // Add conductorId if found
            if (conductorIdStr != null)
            {
                claims.Add(new Claim("conductorId", conductorIdStr));
            }
        ...
        // Survey Sharing Proxy Endpoints
        [HttpPost("surveys/{surveyId}/sharing/enable")]
        public async Task<IActionResult> EnableSharing(int surveyId, [FromBody] object sharingData)
        {
            return await ProxyRequest("POST", $"api/v1/surveys/{surveyId}/sharing/enable", sharingData);
        }

        [HttpGet("surveys/{surveyId}/sharing")]
        public async Task<IActionResult> GetSharingInfo(int surveyId)
        {
            return await ProxyRequest("GET", $"api/v1/surveys/{surveyId}/sharing", null);
        }

        [HttpPut("surveys/{surveyId}/sharing")]
        public async Task<IActionResult> UpdateSharing(int surveyId, [FromBody] object sharingData)
        {
            return await ProxyRequest("PUT", $"api/v1/surveys/{surveyId}/sharing", sharingData);
        }

        [HttpDelete("surveys/{surveyId}/sharing")]
        public async Task<IActionResult> DisableSharing(int surveyId)
        {
            return await ProxyRequest("DELETE", $"api/v1/surveys/{surveyId}/sharing", null);
        }

        [HttpGet("surveys/{surveyId}/sharing/logs")]
        public async Task<IActionResult> GetAccessLogs(int surveyId, [FromQuery] int limit = 50, [FromQuery] int offset = 0)
        {
            return await ProxyRequest("GET", $"api/v1/surveys/{surveyId}/sharing/logs?limit={limit}&offset={offset}", null);
        }

        [HttpPost("surveys/public/validate-access")]
        [AllowAnonymous] // Public endpoint for validating share tokens
        public async Task<IActionResult> ValidateAccess([FromQuery] string token, [FromBody] object? data)
        {
            return await ProxyRequestAnonymous("POST", $"api/v1/surveys/public/validate-access?token={token}", data ?? new { });
        }
        ...
        private async Task<IActionResult> ProxyRequestAnonymous(string method, string endpoint, object? data)
        {
            try
            {
                var surveyServiceUrl = _configuration["SurveyService:BaseUrl"] ?? "http://localhost:3001";
                ...
                // No bearer token for anonymous requests

                var url = $"{surveyServiceUrl}/{endpoint}";

                _logger.LogInformation("Proxying anonymous {Method} request to {Url}", method, url);
            ...
            {
                _logger.LogError(ex, "Unexpected error in anonymous survey proxy");
                return StatusCode(500, ResponseUtil.Error<object>(
                    "Internal server error",
                    "INTERNAL_ERROR",
                    ex.Message));
